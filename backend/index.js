...
// MVP gating by email param (replace with real auth later)
  ...
  try {
    ...
    if (email) {
      ...
      if (u.rowCount) {
        ...
        const s = await pool.query(
          `SELECT status FROM subscriptions WHERE user_id=$1 ORDER BY current_period_end DESC NULLS LAST LIMIT 1`,
          [userId]
        );
        if (s.rowCount && s.rows[0].status === 'active') isPaid = true;
      }
    }
    ...
    if (!isPaid) {
      const limited = rows.slice(0, 10).map(r => ({
        ...
        rocket_score: r.rocket_score
      }));
      return res.json({ paid: false, data: limited });
    }

    return res.json({ paid: true, data: rows });
  } catch (err) {
    console.error(err);
...
app.listen(PORT, async () => {
  console.log(`RocketSignal API listening on ${PORT}`);
  await seedCoins();
});
